/* list.h -- Linked lists taken from the linux kernel and adapted for userspace
 *
...
#ifndef __WIFI_COMMON_LIST_H_
#define __WIFI_COMMON_LIST_H_
						   ...
						   struct list_head *next)
{
	 ...
	 newe->next = next;
	 newe->prev = prev;
	 WRITE_ONCE(prev->next, newe);
}
#else
extern void __list_add(struct list_head *newn,
					   struct list_head *prev,
					   struct list_head *next);
#endif
...
static inline void hlist_move_list(struct hlist_head *old,
                                   struct hlist_head *newe)
{
        newe->first = old->first;
        if (newe->first)
