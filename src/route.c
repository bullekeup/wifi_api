...
int mpath_handler(struct nl_msg *msg, void *arg){
	...
	/*get informations*/
	mac = (char*) nla_data(tab[NL80211_ATTR_MAC]);
	for(i=0;i<ETH_ALEN;i++){
		mp->dest[i] = mac[i];
	}
	mac = (char*) nla_data(tab[NL80211_ATTR_MPATH_NEXT_HOP]);
	for(i=0;i<ETH_ALEN;i++){
		mp->next_hop[i] = mac[i];
	}
...
int wifi_add_mesh_path(char* name, char* dest, char* next_hop, struct wifi_nlstate* nlstate){
	...
	p = new_nlparam(NL80211_ATTR_MAC, TYPE_DATA, ETH_ALEN, dest);
	list_add(&p->entry, &attrs);
	p = new_nlparam(NL80211_ATTR_MPATH_NEXT_HOP, TYPE_DATA, ETH_ALEN, dest);
	list_add(&p->entry, &attrs);
